...
//    }
    ...
    public void intpid(Token next) {
        ss.push(new Address(Integer.parseInt(next.value), varType.Int, new ImidiateAddress()));
    }
    ...
    public void call() {
        ...
        ss.push(temp);
        memory.add3AddressCode(Operation.ASSIGN, new Address(temp.num, varType.Address, new ImidiateAddress()), new Address(symbolTable.getMethodReturnAddress(className, methodName), varType.Address), null);
        memory.add3AddressCode(Operation.ASSIGN, new Address(memory.getCurrentCodeBlockAddress() + 2, varType.Address, new ImidiateAddress()), new Address(symbolTable.getMethodCallerAddress(className, methodName), varType.Address), null);
